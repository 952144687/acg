...
public class AnimeJavaMail {
    // 发件人的 邮箱 和 密码（替换为自己的邮箱和密码）
    // PS: 某些邮箱服务器为了增加邮箱本身密码的安全性，给 SMTP 客户端设置了独立密码（有的邮箱称为“授权码”）,
    //     对于开启了独立密码的邮箱, 这里的邮箱密码必需使用这个独立密码（授权码）。
    public static String sendEmailAccount = "952144687@qq.com";
    public static String sendEmailPwd = "cccovnloletnbeff";
    public static String receiveMailAccount = "952144687@qq.com";
    ...
    public static void sendEmail  (String receiveMailAccount,String content)throws Exception{
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, sendEmailAccount, receiveMailAccount,content);
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,String content) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "ACG资源网", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        //    CC:抄送人，BCC:密送
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "XX用户", "UTF-8"));
        message.setRecipient(MimeMessage.RecipientType.CC, new InternetAddress("952144687@qq.com", "XX用户", "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("新年快乐啊", "UTF-8");
